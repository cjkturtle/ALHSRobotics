...
void MainScreen(){ 
  if(screenClock == 30000){
    ...
    Brain.Screen.printAt(2, 20, "LB Temp: %1.0f" " C", LB.temperature(vex::temperatureUnits::celsius));
    Brain.Screen.printAt(2, 40, "RB Temp: %1.0f" " C", RB.temperature(vex::temperatureUnits::celsius));   
    Brain.Screen.printAt(2, 60, "Tray Temp: %1.0f" " C", Swing.temperature(vex::temperatureUnits::celsius));
    Brain.Screen.printAt(2, 80, "IntakeL Temp: %1.0f" " C", IntakeL.temperature(vex::temperatureUnits::celsius));
    ...
    Brain.Screen.printAt(220, 120, "AY: %1.0f", currentAccelY);
    Brain.Screen.printAt(220, 80, "IntakeLoad: %1.0d", LoadLight.value(analogUnits::pct));


    if(wifiMode == 1){
      Brain.Screen.printAt(2,200, "Wifi Mode: Enabled");
    }
    else{
      Brain.Screen.printAt(2,200, "Wifi Mode: Disabled");
    } 
    if(gameStatus == 1){
      Brain.Screen.printAt(2,140, "Mode: Driver");
    }
    else if(gameStatus == 2){
      Brain.Screen.printAt(2,140, "Mode: Autonomus");
    }
    else { 
      Brain.Screen.printAt(2,140, "Mode: Idle");
    }

    if(controllerStatus == 1){
      Brain.Screen.printAt(220, 20, "Driver Profile: Carter");
    }
    else if(controllerStatus == 2){
      Brain.Screen.printAt(220, 20, "Driver Profile: Other");
    }

    if(!inBeta){
      Brain.Screen.printAt(220, 40, "Beta: Disabled");
    }
    else if(inBeta){
      Brain.Screen.printAt(220, 40, "Beta: Enabled");
    }
  }
...
void AutonSelector(int selectorState){
  if(inBeta){
    Brain.Screen.clearScreen();
    Brain.Screen.printAt(120,20, "BETA!");
  }
  else{
...
void VisionCube(int color, int minSize, int maxSize, int lightThreashold){
  int i = 0;
  bool intake = false;

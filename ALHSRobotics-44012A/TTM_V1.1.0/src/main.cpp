...
void DistMove(double distance, int speed){
  ...
  //LB.startRotateFor(dist,vex::rotationUnits::deg, speed, vex::velocityUnits::pct);
  //RB.rotateFor(-dist,vex::rotationUnits::deg, speed, vex::velocityUnits::pct);
}
...
/* void AccelMove(){
  ...
  vex::task::sleep(accelConst);
} */
void AccelMove(int EndSpeed, int AccelDist){
  int i = 1;
  while(i <= 10){
  ...
  i++;
  vex::task::sleep((AccelDist*28.6)/10);

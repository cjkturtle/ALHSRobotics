...
void MainScreen(){ 
  if(screenClock == 15000){
    ...
    Brain.Screen.printAt(2, 180, "Program: TTM_V2.2.2");
...
// Function for moving they cube tray to either an upright position or to an angled position.
 ...
 void SwingToLocation(int location){
   if((location == 1) && (!swingLimit)){ //go up
    ...
    bool swing = false;
    Swing.spin(vex::directionType::fwd, slowTraySpeed,vex::velocityUnits::pct);
    while(!swing){
  ...
  Swing.stop();
    ...
    float dist = 38.197; //test value
    int swingMotorCurrent = Swing.rotation(vex::rotationUnits::deg);
    bool swing = false;
    Swing.spin(vex::directionType::rev, slowTraySpeed,vex::velocityUnits::pct);
    while(!swing){
    swingMotorCurrent = Swing.rotation(vex::rotationUnits::deg);
    Brain.Screen.printAt(2,2, "SML: %1000l", swingMotorCurrent);
    if(swingMotorCurrent >= dist){
...
void AutonSelector(int selectorState){
  ...
  Brain.Screen.printAt(80,20, "Auton Selection: None");
    ...
    if(Brain.Screen.pressing()){
      ...
      int yPressPos = Brain.Screen.yPosition();
        ...
        if((xPressPos >= 2) && (xPressPos <= 60) && (yPressPos >= 2) && (yPressPos <= 40)){
          autonStatus = 1;
          Brain.Screen.clearScreen();
          Brain.Screen.printAt(80,20, "Auton Selection: Blue Left");
          selectorState--;
          break;
        }
          ...
          autonStatus = 2;
          Brain.Screen.clearScreen();
          Brain.Screen.printAt(80,20, "Auton Selection: Blue Right");
          selectorState--;
          break;
        }
          ...
          autonStatus = 3;
          Brain.Screen.clearScreen();
          Brain.Screen.printAt(80,20, "Auton Selection: Red Left");
          selectorState--;
          break;
        }
          ...
          autonStatus = 4;
          Brain.Screen.clearScreen();
          Brain.Screen.printAt(80,20, "Auton Selection: Red Right");
          selectorState--;
          break;
        }
        else{ 

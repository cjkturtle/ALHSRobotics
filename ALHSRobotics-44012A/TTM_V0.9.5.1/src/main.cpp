...
void MainScreen(){
  Brain.Screen.clearScreen(); 
      ...
      Brain.Screen.printAt(2, 180, "Program: TTM V0.9.4.2");
    ...
    if(Vision1.largestObject.exists && Vision1.largestObject.width>5){
      ...
      Brain.Screen.drawRectangle(221,2, 240,50);
    }   
       Vision1.takeSnapshot(O);
    if(Vision1.largestObject.exists && Vision1.largestObject.width>5){
      Brain.Screen.setFillColor(vex::color::purple);    
      Brain.Screen.drawRectangle(241,2, 260,50);
    } */ 
  }    
...
void DistMove(int distance, int speed){
...
void Degturn(float degrees, int speed){
    int deg = 300*degrees*4;
